...
class FivetranBigQueryDestinationSetup:
    ...
    def __init__(self, config_file_path: str = None):
        ...
        print(
            f"[DEBUG] API Key: {self.api_key[:10]}...{self.api_key[-10:] if len(self.api_key) > 20 else '***'}"
        )
    ...
    def _load_config(self, config_file_path: str = None) -> Dict:
        ...
        try:
            with open(config_file_path, "r") as f:
                ...
                print(
                    f"[INFO] Configuration file loaded successfully from: {config_file_path}"
                )
                config = json.loads(config_content)
                print(
                    f"[DEBUG] Configuration keys found: {list(config.keys())}")
                if 'fivetran' in config:
                    print(
                        f"[DEBUG] Fivetran configuration keys: {list(config['fivetran'].keys())}"
                    )
                return config
    ...
    def _make_api_request(self,
        ...
        try:
            ...
            if response.status_code >= 400:
                print(
                    f"[ERROR] API Error {response.status_code}: {response.text}"
                )
            ...
            if isinstance(response_data, str):
                print(
                    f"[WARNING] API returned string instead of JSON: {response_data}"
                )
                return None
    ...
    def create_bigquery_destination(self,
                                    group_id: str,
                                    project_id: str,
        ...
        print(
            Fore.CYAN +
            f"[INFO] Creating BigQuery destination for project: {project_id}")
        ...
        optional_params = [
            "hybrid_deployment_agent_id", "private_link_id", "proxy_agent_id",
        ...
        if response:
            ...
            print(
                f"[SUCCESS] BigQuery destination created successfully with ID: {destination_id}"
            )
            print(Fore.GREEN +
                  f"✅ BigQuery destination creation completed successfully!")
    ...
    def test_destination_connection(self,
                                    destination_id: str) -> Optional[Dict]:
        """Test the connection to a BigQuery destination."""
        print(f"[INFO] Testing connection for destination: {destination_id}")

        response = self._make_api_request(
            'POST', f'destinations/{destination_id}/test', payload={})
    ...
    def create_group(self, group_name: str) -> Optional[Dict]:
        ...
        print(
            f"[DEBUG] Group creation payload: {json.dumps(payload, indent=2)}")
    ...
    def test_api_connection(self) -> Optional[Dict]:
        ...
        return self._make_api_request('GET', 'groups')
...
def main():
    ...
    config_file = 'fivetran-config.json'
    group_id = 'anointer_indexes'
    group_name = "SmarterDoc"  # Name for the new group
    project_id = "practical-actor-473818-e5"  # Google Cloud Project ID
    dataset_location = "US"  # BigQuery dataset location
    region = "GCP_US_EAST4"  # Fivetran region
    ...
    try:
        ...
        if test_response:
            ...
            return
        print(
            f"[INFO] Bypassing group creation. Reusing existing Group ID: {group_id} (Name: {group_name})"
        )
        # Create group first
        ...
        #     print(
        #         Fore.RED +
        #         "❌ Group creation failed! Cannot proceed with destination creation."
        #     )
        #     return
        ...
        print(Fore.YELLOW + "\n=== Checking Existing BigQuery Destination ===")

        destination_response = destination_setup.get_destination_status(
            destination_id)
        ...
        if destination_response:
            ...
            if destination_id:
                connection_test = destination_setup.test_destination_connection(
                    destination_id)
                if connection_test:
                    print(
                        Fore.GREEN +
                        f"✅ Destination {destination_id} exists and retrieved!"
                    )
                else:
                    print(
                        Fore.YELLOW +
                        f"⚠️ Destination {destination_id} created but connection test failed"
                    )

            # Get destination status
            print(Fore.YELLOW + "\n=== Destination Status ===")
            if destination_id:
                status = destination_setup.get_destination_status(
                    destination_id)
                if status:
                    print(
                        f"[INFO] Destination Status: {status.get('data', {}).get('status', 'Unknown')}"
                    )
                    print(
                        f"[INFO] Service: {status.get('data', {}).get('service', 'Unknown')}"
                    )
                    print(
                        f"[INFO] Region: {status.get('data', {}).get('region', 'Unknown')}"
                    )
        ...
        if destinations:
            ...
            for dest in dest_list:
                print(
                    f"[INFO] - {dest.get('id', 'Unknown')}: {dest.get('service', 'Unknown')} ({dest.get('status', 'Unknown')})"
                )
        else:
            print(
                "[INFO] No destinations found or unable to retrieve destination list"
            )
        ...
        print(Fore.CYAN + f"\nNext steps:")
        print(
            f"1. Configure your Google Sheets connectors to use this destination"
        )
